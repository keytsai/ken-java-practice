...
@Service @Transactional public class MemoServiceImpl implements MemoService {

  @Autowired private MemoRepository memoRepository;

  @Override public List<MemoImpl> getAllMemo() {
    return memoRepository.findAll();
  }

  @Override public Memo save(MemoImpl memoImpl) {
    memoImpl.setTimestamp(LocalDateTime.now());
    return memoRepository.save(memoImpl);
  }

  @Override public Memo findById(Long id) {
    Optional<MemoImpl> memoImpl = memoRepository.findById(id);
    return memoImpl.orElse(null);
  }

  @Override public Memo update(MemoImpl memoImpl) {
    memoImpl.setTimestamp(LocalDateTime.now());
    return memoRepository.save(memoImpl);
  }
